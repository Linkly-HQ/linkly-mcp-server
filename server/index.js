...
async function apiRequest(method, path, body) {
  ...
  const options = {
    ...
    headers: {
      ...
      // FIXED: Swapped these - API key should be in X-API-Key header
      "X-API-Key": process.env.API_Key,
      // Note: Based on your curl, workspace ID goes in the URL path, not header
      // If you need it in header, it would be:
      
    },
  };

  // FIXED: Actually use the body parameter instead of hard-coded value
  if (body) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  // FIXED: Extract args from request
    ...
    switch (request.params.name) {
      case "ping": {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Hello, it's working fine with message: ${message}`,
            },
          ],
        };
      }
      ...
      case "create_link": {
        ...
        const result = await apiRequest(
          ...
          args  // FIXED: Pass the actual args instead of hard-coded body
        );
        return {
          content: [
            {
...
console.error("Linkly MCP server running...");